...
let steps = ref 0
let function_store = Hashtbl.create 100 (** Contains the functions. *)

let break = ref false (** The break flag. *)
let continue = ref false (** The continue flag. *)
  ...
  (match op with
   | Plus | Minus | Times | Divide | Modulus ->
     (match v1, v2 with
      ...
      | Tuple(v1), Tuple(v2) -> Tuple(tuple_operator [] op (Tuple(v1)) (Tuple(v2)))
      | _ -> raise (TypeError ("This operator requires either two integers or two floats: ", Formatter.string_of_operator op)))
   | Less | Leq | Greater | Geq | Eq | Noteq ->
     (match v1, v2 with
      ...
      | _ -> raise (TypeError ("This operator requires either two integers, two floats, two booleans, two strings or two tuples: ", Formatter.string_of_operator op)))
   | And ->
     (match v1 with
...
and
  ...
  if(length parameters != length vs) then raise (FunctionError "This function application doesn't have the required number of parameters")
    ...
    ((combine parameters (map (fun location -> Pointer(location)) locations))@env, extend_list store (combine locations vs), exp)
and
  ...
  | Seq(e1, e2) ->
    ...
    if(!break == true || !continue == true) then v1
      ...
      if(!break == true || !continue == true) then v1 else v2
  | Negate(e) ->
    ...
    (match eval_exp env store e1, eval_exp env store e2 with
     | Integer(i), Integer(j) ->
       if(i <= j) then
         ...
         if(!break == true) then (break := false; eval_exp env store Nothing)
         else (continue := false; eval_exp env store (For(s, MyInteger(i+1), MyInteger(j), e3)))
       else eval_exp env store Nothing
     | _ -> raise (TypeError ("For loops only contains integers", "")))
  | While(e1, e2)->
    let v1 = eval_exp env store e1 in
    if(get_boolean v1) then
      let _ = eval_exp env store e2 in
      if(!break == true) then (break := false; eval_exp env store Nothing)
      else (continue := false; eval_exp env store (While(e1, e2)))
    else eval_exp env store Nothing
  | If(e1, e2, e3) ->
    let v1 = eval_exp env store e1 in
  ...
  | Break -> break := true; eval_exp env store Nothing
  | Continue -> continue := true; eval_exp env store Nothing
...
let eval program =
  ...
  if(!break == true || !continue == true) then failwith "Break or continue outside of a loop"
  else (!steps, evaluation)