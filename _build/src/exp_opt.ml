...
and opt_exp max_loop is_function is_loop store expression = match expression with
	...
	| Asg(e1, e2) ->
  	...
  	| Seq(e1, e2) ->
		(match opt_exp max_loop is_function is_loop store e1 with
			| Nothing | Break | Continue->
				if(!break == true || !continue) then Nothing
				else
			 ...
			 | MyInteger(i), MyInteger(n) ->
				...
				if(i+global_max_loop<n) then
					...
					if(!break == true) then (break := false; Let(s, MyInteger(i), e3))
						...
						(continue := false;
			 			Let(s, MyInteger(i), Seq(opt_exp 0 is_function true store e, For(s, MyInteger(i+global_max_loop), MyInteger(n), e3)))))
				else
  	...
  	| While(e1, e2) ->
		...
		if(max_loop > 0) then
			if(is_primary v1) then
				if(get_boolean v1) then
					let v2 = opt_exp 0 is_function true store e2 in
		...
		else (totally_unrolled := false; While(e1, e2))
  	| If(e1, e2, e3) ->
		(match opt_exp max_loop is_function is_loop store e1 with
		...
		if(fold_left (fun a v -> contains_print v || a) false vs) then Application(opt_exp max_loop true is_loop (Hashtbl.create 100) e, ps)
					...
					if(Hashtbl.mem function_store s) then
				  		(match access function_store s with
					 		| (s, ps, expression) ->
								...
								let res = (try opt_exp max_loop true is_loop (Hashtbl.create 100) (create_nested_new ps vs expression) with
									| Exp_errors.FunctionError msg -> success := false; Application(Identifier(s), vs)) in
								if(!success) then opt_exp max_loop is_function is_loop store res else res
			  	...
			  	| Lambda(pps, expression) ->
					if(length pps > length vs) then
						...
						Application(Lambda(ps2, opt_exp max_loop is_function is_loop (extend_list (Hashtbl.create 100) (combine ps1 vs)) expression), [])
					else
				  		opt_exp max_loop true is_loop (Exp_store.extend_list (Hashtbl.create 100) (combine pps vs)) expression
			  	| v -> Application(v, vs))
